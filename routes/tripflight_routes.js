...
router.get('/edit2', function(req, res){
    if(req.query.country_namme == null) {
        res.send('An email is required');
    }
    else {
        tripflight_dal.edit(req.query.country_name, function(err, result){
            res.render('tripflight/tripflightUpdate', {tripflight: result[0][0]});
        });
    }
...
router.get('/update', function(req, res){
    tripflight_dal.update(req.query, function(err, result){
        res.redirect(302, '/tripflight/all');
    });
});
